		...
		<?php include 'include/header.xhtml';?>
			...
			<div id="content-wrap">
				...
				<main>
					...
					<?php 
					if ($querySuccess) {
						echo '<table><tr><th>Title</th><th>Genre</th><th>Rating</th><th>Action</th><th>Poster</th></tr>';
						foreach ( $statement as $row ) {
							...
							echo '</td>';
							echo '<td>';
							echo '<button type="button" onclick="deleteMovie(\''.$row['movieTitle'].'\')" data-title="'.$row['movieTitle'].'">Remove</button>';
							echo '</td>';
							echo '<td>';
							echo '';
							echo '</td>';
		...
		function deleteMovie(movieTitle) {
			...
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status ==200) {
					$("button[data-title='"+movieTitle+"']").replaceWith("Removed.");
				} else if (this.readyState == 4 && this.status == 500){
					$("button[data-title='"+movieTitle+"']").replaceWith("Sorry.");
				}
			}
